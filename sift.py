...
def sift_detect(img1, img2, detector='surf'):
    ...
    img8bit=cv2.normalize(img1,None,0,255,cv2.NORM_MINMAX).astype('uint8')
    img8bit2=cv2.normalize(img1,None,0,255,cv2.NORM_MINMAX).astype('uint8')
    kp1, des1 = sift.detectAndCompute(img8bit, None)
    kp2, des2 = sift.detectAndCompute(img8bit2, None)
...
if __name__ == "__main__":
    # load image
    image_a = cv2.imread('test_images\\LenaRGB.bmp')
    image_b = cv2.imread('result_images\\LenaRGB.modified9.bmp')
    # ORB
